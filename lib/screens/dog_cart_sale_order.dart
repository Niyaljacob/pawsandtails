...
class DogCartSaleOrder extends StatefulWidget {
...
class _DogCartSaleOrderState extends State<DogCartSaleOrder> {
  int _currentStep = 0;
          ...
          builder: (context, snapshot) {
            ...
            for (var doc in snapshot.data!.docs) {
              ...
              try {
                ...
                productsInCart.add({
                  'name': productData['dogName'],
                  'imageUrls': productData['userEmail']?[0] ??
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Dismissible(
                      ...
                      child: ListTile(
                        ...
                        title: Text(productData['dogName'] ??
  ...
  Future<void> _storeOrderDetails(
    ...
    try {
      await FirebaseFirestore.instance.collection('dog_cart_payment').add({
        'userEmail': userEmail,
    ...
    try {
      await FirebaseFirestore.instance.collection('dog_cart_payment').add({
        ...
        'products': productsInCart.map((product) {
          return {
            'name': product['name'],
            'imageUrls': product['imageUrls'], // Ensure imageUrls are added
          };
        }).toList(),
        'fullName': _fullNameController.text,
        'email': _emailController.text,
